<template lang="pug">
a-drawer(width=640 title='Документ', placement='right', @close='close', :visible='isOpen' :wrapStyle="{ height: '100%' }")
  .inner
...
export default {
  methods: {
    close() {
      this.$store.commit('closeDocViewer')
    }
  },
  computed: {
    isOpen(){
      return this.$store.state.docViwerDrawer
    },
    doc() {
      return this.$store.state.actualDoc
    }
  }
}
</script>
